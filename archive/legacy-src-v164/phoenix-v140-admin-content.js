...
(function initPhoenixV140(){
  ...
  function normalizeSocialLink(item = {}, index = 0){
    ...
    return {
      id: item.id || uid('social'),
      platform,
      label: item.label || item.linkLabel || platform,
      url: normalizeUrl(item.url || item.link || ''),
      qr: item.qr || item.image || '',
  ...
  function socialLinkCardMarkup(item){
    ...
    const qr = item.qr && isSafeImageSrc(item.qr) ? `<img class="social-qr-img" src="${esc(item.qr)}" alt="${esc(item.platform)} QR code">` : `<span class="social-qr-fallback">QR</span>`;
    return `<article class="social-link-card"><a href="${esc(href)}" target="${target}" rel="noopener noreferrer"><strong>${esc(item.platform)}</strong><span>${esc(item.label || item.platform)}</span><small>${esc(item.note || '')}</small></a><div class="social-qr-box">${qr}</div></article>`;
  }
  ...
  function pricingPage(){
    ...
    return `<div class="v140-admin-panel"><div class="section-row"><div><h3>Pricing / Menu Settings</h3><p class="small-muted">Change prices once. Homepage, booking calculation, invoice, payment panel, and revenue view will read the same pricing source in this browser. Supabase sync can make it global later.</p></div><button type="button" class="outline-btn" data-v140-reset-pricing>Reset defaults</button></div>
      <div class="v140-settings-grid">
        ...
        <article><h4>Add-ons</h4><p class="small-muted">For adding/deleting photos and notes, use the Add-ons Menu Manager tab.</p>${Object.entries(addons).map(([k,v]) => num(`addons.${k}`, v, k)).join('')}</article>
