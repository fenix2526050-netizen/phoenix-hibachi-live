...
function paymentAmount(row:Row,paymentType:string){
  const balance=Math.max(0,Number(row.balance_due_cents ?? row.order_total_cents ?? 0))
  if(paymentType==='full_balance') return balance
  const depositDue=Math.max(0,Number(row.deposit_due_cents ?? row.deposit_required_cents ?? 20000))
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(!['deposit','full_balance'].includes(type))throw new Error('Choose either the $200 deposit or the full remaining balance')
    ...
    const amountDue=paymentAmount(row,type)
    ...
    if(type==='deposit'&&amountDue>20000)throw new Error('The deposit amount exceeds the approved $200 online deposit')
    ...
    const productName=type==='full_balance'?'Phoenix Hibachi Full Booking Balance':'Phoenix Hibachi Booking Deposit'
    const description=type==='full_balance'?`Full remaining balance for ${row.booking_number}`:`$200 deposit for ${row.booking_number}`
    ...
    const pendingStatus=type==='full_balance'?'full_payment_pending':'awaiting_webhook'
