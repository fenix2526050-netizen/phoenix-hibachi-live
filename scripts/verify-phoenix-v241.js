...
check('V241 script is cache-busted to force latest modify UI', includes(index, 'phoenix-v241-order-modification.js?v=244') && includes(v2382, 'phoenix-v241-order-modification.js?v=244'));
...
check('V241 customer modify caps total protein pool, not each protein by guest count', includes(v241, 'maxTotal:required') && includes(v241, 'clampProteinTotal') && includes(v241, 'cannot exceed ${rule.required} total portions') && !includes(v241, 'maxEach'));
...
check('V241 mobile lookup modal is scrollable and usable', includes(v241, 'max-height:92dvh') && includes(v241, '#orderLookupModal input{font-size:16px}') && includes(v241, 'position:sticky;bottom:0'));
